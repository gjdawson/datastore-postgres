...
import {hashCode, LockManager, logger, pause} from "@eventicle/eventicle-utilities";
import {getFileNameAndLineNumber} from "@eventicle/eventicle-utilities/dist/logger-util";
import {PsqlEventDataStore} from "./index";
...
export class PSQLLockManager<E extends Error> implements LockManager {

  constructor(readonly db: PsqlEventDataStore<E>) {
  }
  ...
  private async internalLock<T>(origId: string, onLock: () => Promise<T>, teardown: () => void, maxAttempts: number): Promise<T> {
    ...
    let exec = async () => {
      const trx: ITask<any> = als.get("transaction")
        ...
        try {
          if (doUnlock) {
            ...
            for (let attempt = 0; attempt < 10; attempt++) {
              const unlocked = await trx.any(`select pg_advisory_unlock(${PG_ADVISE_MAGIC}, ${id});`)
              if (unlocked[0].pg_advisory_unlock) {
